...
RUN --mount=type=secret,id=GIT_AUTH_TOKEN,dst=/tmp/secret_token export GITHUB_TOKEN="$(cat /tmp/secret_token)" \
    ...
    && git clone -j "$(nproc)" "https://IceCodeNew:${GITHUB_TOKEN}@github.com/IceCodeNew/qBittorrent-Enhanced-Edition.git" . \
    ...
    && git tag -d "$TAG_NAME"; \
    git tag -a "$TAG_NAME" -m "$TAG_NAME" "$(git rev-parse --short origin/release)" \
